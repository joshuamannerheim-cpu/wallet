...
VERSION = "5.2.2-quality-alts-mobile"
...
SOLANA_PORTFOLIO_WALLET = os.getenv(
    "SOLANA_PORTFOLIO_WALLET", "346ef9A3bkfGno5b7uQRn89ZhiPJFVCAma98VXYmyKrD"
).strip()
...
EVM_EARLY_BUYER_LOG_BLOCKS = min(
    max(int(os.getenv("EVM_EARLY_BUYER_LOG_BLOCKS", "40000")), 2000), 100000
)
EVM_EARLY_BUYER_LOG_CHUNK = min(
    max(int(os.getenv("EVM_EARLY_BUYER_LOG_CHUNK", "2000")), 100), 5000
)
EVM_EARLY_BUYER_MAX_RECEIPTS = min(
    max(int(os.getenv("EVM_EARLY_BUYER_MAX_RECEIPTS", "400")), 50), 750
)
...
def initialise_database():
    ...
    with db() as conn:
        with conn.cursor() as cur:
            ...
            """)
            cur.execute("ALTER TABLE evm_wallet_holdings ADD COLUMN IF NOT EXISTS token_name TEXT")
            cur.execute("ALTER TABLE evm_wallet_holdings ADD COLUMN IF NOT EXISTS decimals INTEGER")
            cur.execute("ALTER TABLE evm_wallet_holdings ADD COLUMN IF NOT EXISTS ui_balance DOUBLE PRECISION")
            cur.execute("ALTER TABLE evm_wallet_holdings ADD COLUMN IF NOT EXISTS usd_value DOUBLE PRECISION")
            cur.execute("ALTER TABLE evm_wallet_holdings ADD COLUMN IF NOT EXISTS discovery_source TEXT")
            cur.execute("""
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS evm_early_buyer_runs (
                    id BIGSERIAL PRIMARY KEY,
                    started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                    completed_at TIMESTAMPTZ,
                    tokens_selected INTEGER NOT NULL DEFAULT 0,
                    tokens_completed INTEGER NOT NULL DEFAULT 0,
                    wallets_observed INTEGER NOT NULL DEFAULT 0,
                    status TEXT NOT NULL DEFAULT 'running',
                    stop_reason TEXT,
                    details TEXT NOT NULL DEFAULT '{}'
                )
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS evm_early_buyer_token_state (
                    chain TEXT NOT NULL,
                    token_address TEXT NOT NULL,
                    token_symbol TEXT NOT NULL,
                    pair_address TEXT,
                    pair_created_at TIMESTAMPTZ,
                    pair_created_block BIGINT,
                    last_scanned_block BIGINT,
                    observations_retained INTEGER NOT NULL DEFAULT 0,
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS evm_early_buyer_observations (
                    chain TEXT NOT NULL,
                    wallet TEXT NOT NULL,
                    token_address TEXT NOT NULL,
                    token_symbol TEXT NOT NULL,
                    ...
                    seconds_after_pair DOUBLE PRECISION,
                    token_amount DOUBLE PRECISION,
                    occurred_at TIMESTAMPTZ,
                    filter_status TEXT NOT NULL DEFAULT 'eligible',
                    exclusion_reason TEXT,
                    evidence_quality TEXT NOT NULL DEFAULT 'pair_verified',
                    details TEXT NOT NULL DEFAULT '{}',
                    observed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            ...
            # V4.14.4 retires PORTLY from active monitoring while preserving
            # its historical snapshots, signals and transition evidence.
            cur.execute("""
                UPDATE token_watchlist
                SET active = FALSE,
                    monitoring_status = 'removed_from_watchlist_v4_14_4',
                    updated_at = NOW()
...
def diagnostics():
    ...
    return jsonify({"success": True, "version": VERSION, "premium_mode": False,
                    ...
                    "portfolio_holdings": {
                        "wallets": {
                            "evm": EVM_PORTFOLIO_WALLET,
                            "solana": SOLANA_PORTFOLIO_WALLET,
                        },
                        "read_only": True,
                        "scheduled_with_evm_refresh": True,
                        "refresh_hours": EVM_HOLDINGS_REFRESH_HOURS,
                        "scope": "all discovered nonzero wallet holdings",
                        "discovery": {
                            "robinhood_and_base": "Blockscout plus GMGN activity",
                            "bsc": "GMGN activity plus public RPC",
                            "solana": "Helius DAS",
                        },
                    },
                    ...
                    "evm_early_buyer_discovery": {
                        ...
                        "pair_event_verification_required": True,
                        "same_block_snipers_excluded": True,
...
def load_dex_wallet_pipeline_status():
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT candidate.wallet, candidate.score,
                    ...
                    candidate.early_entry_score, candidate.tokens_found,
                    candidate.total_pnl_30d, candidate.win_rate_30d,
                    candidate.total_invested_30d, candidate.score_status,
                    candidate.screening_status,
                    COALESCE(screening.risk_flags, '[]')
                FROM candidate_wallets candidate
    ...
    names = (
        ...
        "minimum_gates_passed", "probation", "consensus_validated",
    )
    ...
    for item in ready_rows:
        wallet = item[0]
        try:
            risk_flags = json.loads(item[14] or "[]")
        except (TypeError, ValueError):
            risk_flags = []
        candidate = {
            "wallet": wallet, "score": item[1], "realized_pnl": item[3],
            "trades": item[4], "validation_status": item[5],
        ...
        classification = classify_candidate(
            candidate,
            {"risk_score": item[2], "risk_flags": risk_flags},
            validation, evidence,
        )
        ...
        if (
            ...
            and classification["classification"] in {"WATCH", "ASYMMETRIC"}
    ...
    return {
        ...
        "recommended_probation_wallets": recommended_wallets,
        "policy": {
            "batch_limits": {"score": 5, "screen": 5, "validate": 2},
            "recommendation_requires": {
                "validation_strength": "strong",
                "classification": ["WATCH", "ASYMMETRIC"],
                "repeat_or_cross_token_early_entry_evidence": True,
            },
...
def robinhood_rpc(method, params):
    """Call the official read-only RPC once; provider failures are never retried aggressively."""
    response = upstream_request(
        "POST", ROBINHOOD_RPC_URL,
        headers={"content-type": "application/json"},
        json_body={"jsonrpc": "2.0", "id": 1, "method": method, "params": params},
        timeout=EVM_PROVIDER_TIMEOUT_SECONDS, retries=0, provider="robinhood_rpc",
    )
    if response.status_code != 200:
        raise RuntimeError(f"robinhood_rpc_http_{response.status_code}")
    payload = response.json()
    ...
    for rpc_url in rpc_urls:
        try:
            response = upstream_request(
                ...
                timeout=EVM_HOLDINGS_RPC_TIMEOUT_SECONDS, retries=0,
...
def gmgn_activity_tokens(chain, wallet, pages=3):
    """Discover recently active token contracts with API-key-only GMGN reads."""
    if not os.getenv("GMGN_API_KEY"):
        return []
    cursor = None
    discovered = {}
    for _ in range(min(max(int(pages), 1), 5)):
        params = {
            "chain": chain, "wallet_address": wallet, "limit": 100,
        }
        if cursor:
            params["cursor"] = cursor
        data = gmgn_request("GET", "/v1/user/wallet_activity", params=params)
        ...
        for activity in data.get("activities") or []:
            ...
            address = str(token.get("address") or token.get("token_address") or "").strip()
            ...
            try:
                ...
                continue
            discovered[address.lower()] = {
                "chain": chain, "token_address": address,
                "token_symbol": str(token.get("symbol") or "UNKNOWN")[:32],
                "token_name": str(token.get("name") or token.get("symbol") or "Unknown")[:120],
                "decimals": safe_int(token.get("decimals")),
                "source": "gmgn_activity",
            }
...
def blockscout_token_holdings(chain, wallet):
    """Discover the explorer's complete current ERC-20 balance inventory."""
    base_url = (EVM_CHAIN_CONFIG.get(chain) or {}).get("blockscout_url")
    if not base_url:
        return []
    response = upstream_request(
        "GET", f"{base_url}/addresses/{wallet}/token-balances",
        headers={"accept": "application/json", "user-agent": f"wallet-monitor/{VERSION}"},
        timeout=EVM_HOLDINGS_RPC_TIMEOUT_SECONDS, retries=1,
        provider=f"{chain}_blockscout",
    )
    if response.status_code != 200:
        raise RuntimeError(f"{chain}_blockscout_http_{response.status_code}")
    payload = response.json()
    rows = payload.get("items") if isinstance(payload, dict) else payload
    if not isinstance(rows, list):
        raise RuntimeError(f"{chain}_blockscout_invalid_response")
    holdings = []
    for row in rows:
        ...
        if token_type and token_type not in {"ERC-20", "ERC20"}:
            continue
        address = str(
            token.get("address_hash") or token.get("address")
            or row.get("token_address") or ""
        ).strip()
        raw_balance = safe_int(row.get("value") or row.get("balance"))
        if (
            len(address) != 42 or not address.lower().startswith("0x")
            or raw_balance is None or raw_balance <= 0
        ):
            continue
        try:
            int(address[2:], 16)
        ...
        holdings.append({
            ...
            "token_symbol": str(token.get("symbol") or "UNKNOWN")[:32],
            "token_name": str(token.get("name") or token.get("symbol") or "Unknown")[:120],
            "decimals": decimals, "raw_balance": str(raw_balance),
            "ui_balance": (
                raw_balance / (10 ** decimals) if decimals is not None else None
            ),
            "source": "blockscout_token_balances",
        })
    return holdings


def helius_solana_holdings(wallet):
    """Read all current fungible assets for a Solana owner through Helius DAS."""
    api_key = os.getenv("HELIUS_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("helius_not_configured")
    holdings = {}
    page = 1
    while page <= 5:
        response = upstream_request(
            "POST", f"https://mainnet.helius-rpc.com/?api-key={api_key}",
            headers={"content-type": "application/json"},
            json_body={
                "jsonrpc": "2.0", "id": page, "method": "getAssetsByOwner",
                "params": {
                    "ownerAddress": wallet, "page": page, "limit": 100,
                    "displayOptions": {
                        "showFungible": True, "showNativeBalance": True,
                    },
                },
            },
            timeout=25, retries=1, provider="helius",
        ...
        if response.status_code != 200:
            raise RuntimeError(f"helius_assets_http_{response.status_code}")
        payload = response.json()
        if not isinstance(payload, dict) or payload.get("error"):
            error = payload.get("error") if isinstance(payload, dict) else None
            code = error.get("code") if isinstance(error, dict) else "invalid_response"
            raise RuntimeError(f"helius_assets_error_{code}")
        result = payload.get("result") or {}
        items = result.get("items") or []
        for asset in items:
            if not isinstance(asset, dict):
                continue
            interface = str(asset.get("interface") or "")
            if interface and interface not in {"FungibleToken", "FungibleAsset"}:
                continue
            token_info = asset.get("token_info") or {}
            raw_balance = safe_int(token_info.get("balance"))
            if raw_balance is None or raw_balance <= 0:
                continue
            mint = str(asset.get("id") or "").strip()
            if not mint:
                continue
            decimals = safe_int(token_info.get("decimals")) or 0
            ui_balance = raw_balance / (10 ** decimals)
            metadata = (asset.get("content") or {}).get("metadata") or {}
            price_info = token_info.get("price_info") or {}
            usd_value = safe_float(price_info.get("total_price"))
            if usd_value is None:
                price = safe_float(price_info.get("price_per_token"))
                usd_value = ui_balance * price if price is not None else None
            holdings[mint] = {
                "chain": "solana", "wallet": wallet,
                "token_address": mint,
                "token_symbol": str(metadata.get("symbol") or "UNKNOWN")[:32],
                "token_name": str(metadata.get("name") or metadata.get("symbol") or "Unknown")[:120],
                "raw_balance": str(raw_balance), "decimals": decimals,
                "ui_balance": ui_balance, "usd_value": usd_value,
                "source": "helius_das",
            }
        native = result.get("nativeBalance") or result.get("native_balance") or {}
        lamports = safe_int(native.get("lamports"))
        if lamports and lamports > 0:
            price = safe_float(native.get("price_per_sol") or native.get("price"))
            holdings[SOL_MINT] = {
                "chain": "solana", "wallet": wallet,
                "token_address": SOL_MINT, "token_symbol": "SOL",
                "token_name": "Solana", "raw_balance": str(lamports),
                "decimals": 9, "ui_balance": lamports / 1_000_000_000,
                "usd_value": safe_float(native.get("total_price")) or (
                    lamports / 1_000_000_000 * price if price is not None else None
                ),
                "source": "helius_das",
            }
        if len(items) < 100:
...
def persist_portfolio_holding(run_id, holding):
    """Upsert one current holding and record only held/not-held transitions."""
    wallet = holding["wallet"]
    chain = holding["chain"]
    token_address = holding["token_address"]
    token_symbol = holding["token_symbol"]
    raw_balance = str(holding["raw_balance"])
    is_held = safe_int(raw_balance) is not None and safe_int(raw_balance) > 0
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT raw_balance, is_held FROM evm_wallet_holdings
                WHERE wallet = %s AND chain = %s AND token_address = %s
            ...
            cur.execute("""
                INSERT INTO evm_wallet_holdings (
                    wallet, chain, token_address, token_symbol, token_name,
                    raw_balance, decimals, ui_balance, usd_value,
                    discovery_source, is_held, last_changed_at
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                    CASE WHEN %s THEN NOW() ELSE NULL END)
                ON CONFLICT (wallet, chain, token_address) DO UPDATE SET
                    token_symbol = EXCLUDED.token_symbol,
                    token_name = COALESCE(EXCLUDED.token_name, evm_wallet_holdings.token_name),
                    raw_balance = EXCLUDED.raw_balance,
                    decimals = COALESCE(EXCLUDED.decimals, evm_wallet_holdings.decimals),
                    ui_balance = COALESCE(EXCLUDED.ui_balance, evm_wallet_holdings.ui_balance),
                    usd_value = EXCLUDED.usd_value,
                    discovery_source = COALESCE(EXCLUDED.discovery_source,
                        evm_wallet_holdings.discovery_source),
            ...
            """, (
                ...
                holding.get("token_name"), raw_balance, holding.get("decimals"),
                holding.get("ui_balance"), holding.get("usd_value"),
                holding.get("source"), is_held, previous is not None,
            ))
            if change_type:
                cur.execute("""
                ...
                """, (
                    run_id, wallet, chain, token_address, token_symbol,
                    previous[0], raw_balance, change_type,
                ))
        conn.commit()
    return is_held, bool(change_type)


def refresh_evm_wallet_holdings(force=False):
    """Discover and refresh EVM plus Solana portfolio holdings twice daily."""
    initialise_database()
    wallet = EVM_PORTFOLIO_WALLET
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT completed_at, details FROM evm_wallet_holding_runs
    ...
    for chain in SUPPORTED_EVM_CHAINS:
        try:
            ...
            discovery_errors.append({
                "chain": chain, "provider": "blockscout",
                "error": str(exc)[:160],
            })
        try:
            for item in gmgn_activity_tokens(chain, wallet):
                key = (chain, item["token_address"].lower())
                if key not in discovered:
                    discovered[key] = item
        except (requests.RequestException, RuntimeError, ValueError, TypeError) as exc:
            discovery_errors.append({
                "chain": chain, "provider": "gmgn_activity",
                "error": str(exc)[:160],
            })
        ...
        with db() as conn:
            with conn.cursor() as cur:
                for item in discovered.values():
                    ...
                    if existing:
                        item["token_address"] = existing[0]
                        cur.execute("""
                            UPDATE token_watchlist SET
                                token_symbol = CASE WHEN %s <> 'UNKNOWN'
                                    THEN %s ELSE token_symbol END,
                                token_name = CASE WHEN %s <> 'Unknown'
                                    THEN %s ELSE token_name END,
                                active = TRUE, updated_at = NOW()
                            WHERE chain = %s AND token_address = %s
                        """, (
                            item["token_symbol"], item["token_symbol"],
                            item["token_name"], item["token_name"],
                            item["chain"], existing[0],
                        ))
                    else:
                        item["token_address"] = item["token_address"].lower()
                        cur.execute("""
                            INSERT INTO token_watchlist (
                                chain, token_address, token_symbol, token_name,
                                source, monitoring_status, active, updated_at
                            ) VALUES (%s, %s, %s, %s, 'portfolio_discovery',
                                'evm_monitoring_ready', TRUE, NOW())
                        """, (
                            item["chain"], item["token_address"],
                            item["token_symbol"], item["token_name"],
                        ))
            conn.commit()
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                SELECT chain, token_address, token_symbol,
                    COALESCE(token_name, token_symbol)
                FROM evm_wallet_holdings
                WHERE wallet = %s AND chain IN ('robinhood', 'base', 'bsc')
                    AND token_address LIKE '0x%%'
    ...
    for chain, token_address, token_symbol, token_name in tokens:
        try:
            ...
            is_held, changed = persist_portfolio_holding(run_id, {
                ...
                "token_address": token_address, "token_symbol": token_symbol,
                "token_name": token_name, "raw_balance": str(balance),
                "decimals": decimals, "ui_balance": ui_balance,
                "usd_value": None,
                "source": metadata.get("source") or "evm_rpc",
            })
            if not is_held:
                with db() as conn:
                    with conn.cursor() as cur:
                        cur.execute("""
                            UPDATE token_watchlist SET active = FALSE, updated_at = NOW()
                            WHERE chain = %s AND token_address = %s
                                AND source = 'portfolio_discovery'
                        """, (chain, token_address))
                    conn.commit()
            checked += 1
            held += int(is_held)
    ...
    for chain in SUPPORTED_EVM_CHAINS:
        ...
        try:
            native_balance = rpc_hex_int(evm_rpc(chain, "eth_getBalance", [wallet, "latest"]))
            if native_balance is None:
                raise RuntimeError(f"{chain}_native_balance_missing")
            is_held, changed = persist_portfolio_holding(run_id, {
                "wallet": wallet, "chain": chain,
                "token_address": "native", "token_symbol": symbol,
                "token_name": "BNB" if chain == "bsc" else "Ether",
                "raw_balance": str(native_balance), "decimals": 18,
                "ui_balance": native_balance / (10 ** 18), "usd_value": None,
                "source": "evm_rpc_native",
            })
            checked += 1
            held += int(is_held)
            changes += int(changed)
            ...
            errors.append({
                "chain": chain, "token_symbol": symbol,
                "error": str(exc)[:160],
            })

    try:
        solana_holdings = helius_solana_holdings(SOLANA_PORTFOLIO_WALLET)
        current_sol_mints = {item["token_address"] for item in solana_holdings}
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT token_address, token_symbol, token_name, raw_balance,
                        decimals, ui_balance, usd_value
                    FROM evm_wallet_holdings
                    WHERE wallet = %s AND chain = 'solana' AND is_held = TRUE
                """, (SOLANA_PORTFOLIO_WALLET,))
                prior_sol = cur.fetchall()
        for item in solana_holdings:
            is_held, changed = persist_portfolio_holding(run_id, item)
            checked += 1
            held += int(is_held)
            changes += int(changed)
        for row in prior_sol:
            if row[0] in current_sol_mints:
                continue
            is_held, changed = persist_portfolio_holding(run_id, {
                "wallet": SOLANA_PORTFOLIO_WALLET, "chain": "solana",
                "token_address": row[0], "token_symbol": row[1],
                "token_name": row[2], "raw_balance": "0",
                "decimals": row[4], "ui_balance": 0, "usd_value": 0,
                "source": "helius_das",
            })
            checked += 1
            changes += int(changed)
    except (requests.RequestException, RuntimeError, ValueError, TypeError) as exc:
        errors.append({"chain": "solana", "error": str(exc)[:160]})

    all_errors = errors + discovery_errors
    status = "completed" if not all_errors else "partial" if checked else "failed"
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
            ...
            """, (
                ...
                json.dumps({
                    "tokens_selected": len(tokens), "errors": errors,
                    "discovered_evm_tokens": len(discovered),
                    "discovery_errors": discovery_errors,
                    "solana_wallet": SOLANA_PORTFOLIO_WALLET,
                    "portfolio_discovery_version": VERSION,
                }), run_id,
            ))
        conn.commit()
    return {
...
def robinhood_block_at_or_after(target_timestamp):
    """Binary-search the public archive RPC for the first block at a UTC timestamp."""
    latest = rpc_hex_int(robinhood_rpc("eth_blockNumber", []))
    if latest is None:
        raise RuntimeError("robinhood_rpc_latest_block_missing")
    low, high = 0, latest
    while low < high:
        middle = (low + high) // 2
        block = robinhood_rpc("eth_getBlockByNumber", [hex(middle), False])
        block_time = rpc_hex_int(block.get("timestamp")) if isinstance(block, dict) else None
        if block_time is None:
            raise RuntimeError("robinhood_rpc_block_timestamp_missing")
        if block_time < int(target_timestamp):
            low = middle + 1
        else:
            high = middle
    return low, latest
...
def robinhood_token_pair(token_address, canonical_pair):
    response = upstream_request(
        "GET", DEXSCREENER_TOKEN_URL.format(address=token_address),
        timeout=EVM_PROVIDER_TIMEOUT_SECONDS, retries=0, provider="dexscreener",
    )
    if response.status_code != 200:
        raise RuntimeError(f"dexscreener_http_{response.status_code}")
    payload = response.json()
    pairs = payload.get("pairs") if isinstance(payload, dict) else []
    pairs = pairs if isinstance(pairs, list) else []
    eligible = [
        pair for pair in pairs if isinstance(pair, dict)
        and str(pair.get("chainId") or "").lower() in EVM_CHAIN_CONFIG["robinhood"]["dex_chain_ids"]
    ...
    if not eligible:
        raise RuntimeError("canonical_pair_metadata_unavailable")
    pair = max(
        eligible,
    ...
    if not created_ms:
        raise RuntimeError("pair_creation_time_unavailable")
    return pair, datetime.fromtimestamp(created_ms / 1000, tz=timezone.utc)


def receipt_uses_pair(receipt, pair_address):
    ...
    for log in receipt.get("logs") or []:
        ...
        if len(pair) == 42 and address == pair:
            return True
        if len(pair) == 66 and pair in topics[1:]:
            return True
    return False


def receipt_token_delta(receipt, token_address, wallet):
    delta = 0
    token = token_address.lower()
    wallet = wallet.lower()
    for log in receipt.get("logs") or []:
        topics = log.get("topics") or []
        if str(log.get("address") or "").lower() != token or len(topics) < 3:
            continue
        if str(topics[0]).lower() != ERC20_TRANSFER_TOPIC:
            continue
        amount = rpc_hex_int(log.get("data")) or 0
...
def scan_robinhood_early_buyers(token, deadline):
    """Reconstruct bounded, pair-verified early buys for one Robinhood token."""
    chain, address, symbol, canonical_pair = token
    pair, pair_created_at = robinhood_token_pair(address, canonical_pair)
    pair_address = str(pair.get("pairAddress") or canonical_pair or "")
    pair_block, latest_block = robinhood_block_at_or_after(
        pair_created_at.timestamp() - 5
    )
    decimals_raw = robinhood_rpc("eth_call", [{"to": address, "data": "0x313ce567"}, "latest"])
    decimals = rpc_hex_int(decimals_raw)
    decimals = decimals if decimals is not None and 0 <= decimals <= 36 else 18
    end_block = min(pair_block + EVM_EARLY_BUYER_LOG_BLOCKS, latest_block)
    transfer_logs = []
    stop_reason = None
    cursor = pair_block
    while cursor <= end_block and time.monotonic() < deadline:
        chunk_end = min(cursor + EVM_EARLY_BUYER_LOG_CHUNK - 1, end_block)
        logs = robinhood_rpc("eth_getLogs", [{
            "address": address, "fromBlock": hex(cursor), "toBlock": hex(chunk_end),
            "topics": [ERC20_TRANSFER_TOPIC],
        }])
        if not isinstance(logs, list):
            raise RuntimeError("robinhood_rpc_logs_invalid")
        transfer_logs.extend(log for log in logs if isinstance(log, dict))
        cursor = chunk_end + 1
    if cursor <= end_block:
        stop_reason = "deadline_guard"
    ...
    ordered_hashes = []
    seen_hashes = set()
    for log in sorted(transfer_logs, key=lambda item: (
    ...
    for tx_hash in ordered_hashes:
        if len(observations) >= EVM_EARLY_BUYER_LIMIT:
            stop_reason = stop_reason or "wallet_limit"
            break
        if receipts_checked >= EVM_EARLY_BUYER_MAX_RECEIPTS:
            stop_reason = stop_reason or "receipt_limit"
            break
        if time.monotonic() >= deadline:
            stop_reason = "deadline_guard"
            break
        receipt = robinhood_rpc("eth_getTransactionReceipt", [tx_hash])
        receipts_checked += 1
        if not isinstance(receipt, dict) or not receipt_uses_pair(receipt, pair_address):
            continue
        ...
        if raw_delta <= 0:
            continue
        block_number = rpc_hex_int(receipt.get("blockNumber"))
        ...
        observations.append({
            ...
            "seconds_after_pair": seconds_after_pair,
            "token_amount": raw_delta / (10 ** decimals),
            "occurred_at": pair_created_at + timedelta(seconds=seconds_after_pair),
            "filter_status": "excluded" if exclusion_reason else "eligible",
            ...
            for item in observations:
                cur.execute("""
                    INSERT INTO evm_early_buyer_observations (
                        chain, wallet, token_address, token_symbol, pair_address,
                        transaction_hash, block_number, entry_rank,
                        seconds_after_pair, token_amount, occurred_at,
                        filter_status, exclusion_reason, evidence_quality, details
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                        %s, %s, 'pair_verified_timing_estimated', %s)
                    ON CONFLICT (chain, wallet, token_address) DO UPDATE SET
                        transaction_hash = CASE WHEN EXCLUDED.entry_rank < evm_early_buyer_observations.entry_rank THEN EXCLUDED.transaction_hash ELSE evm_early_buyer_observations.transaction_hash END,
                        block_number = LEAST(evm_early_buyer_observations.block_number, EXCLUDED.block_number),
                        entry_rank = LEAST(evm_early_buyer_observations.entry_rank, EXCLUDED.entry_rank),
                        seconds_after_pair = LEAST(evm_early_buyer_observations.seconds_after_pair, EXCLUDED.seconds_after_pair),
                        token_amount = CASE WHEN EXCLUDED.entry_rank < evm_early_buyer_observations.entry_rank THEN EXCLUDED.token_amount ELSE evm_early_buyer_observations.token_amount END,
                        occurred_at = LEAST(evm_early_buyer_observations.occurred_at, EXCLUDED.occurred_at),
                        filter_status = CASE WHEN evm_early_buyer_observations.filter_status = 'eligible' OR EXCLUDED.filter_status = 'eligible' THEN 'eligible' ELSE 'excluded' END,
                        exclusion_reason = CASE WHEN evm_early_buyer_observations.filter_status = 'eligible' OR EXCLUDED.filter_status = 'eligible' THEN NULL ELSE EXCLUDED.exclusion_reason END,
                        observed_at = NOW()
                """, (
                    chain, item["wallet"], address, symbol, pair_address,
                    item["transaction_hash"], item["block_number"], item["entry_rank"],
                    item["seconds_after_pair"], item["token_amount"], item["occurred_at"],
                    item["filter_status"], item["exclusion_reason"],
                    json.dumps({"consensus_weight": 0, "timing_is_estimated": True}),
                ))
            cur.execute("""
                INSERT INTO evm_early_buyer_token_state (
                    chain, token_address, token_symbol, pair_address,
                    pair_created_at, pair_created_block, last_scanned_block,
                    observations_retained, scan_status, provider_status,
                    last_error, details, updated_at
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s,
                    'healthy', NULL, %s, NOW())
                ON CONFLICT (chain, token_address) DO UPDATE SET
                    token_symbol = EXCLUDED.token_symbol,
                    ...
                    pair_created_block = EXCLUDED.pair_created_block,
                    last_scanned_block = GREATEST(evm_early_buyer_token_state.last_scanned_block, EXCLUDED.last_scanned_block),
                    observations_retained = EXCLUDED.observations_retained,
                    scan_status = EXCLUDED.scan_status,
                    provider_status = 'healthy', last_error = NULL,
                    details = EXCLUDED.details, updated_at = NOW()
            """, (
                chain, address, symbol, pair_address, pair_created_at,
                pair_block, max(cursor - 1, pair_block), len(observations),
                "complete" if stop_reason in {None, "wallet_limit"} else "bounded_stop",
                json.dumps({"receipts_checked": receipts_checked,
                            "transfer_logs": len(transfer_logs),
                            "stop_reason": stop_reason}),
            ))
        conn.commit()
    return {
        "token_symbol": symbol, "token_address": address,
        "pair_address": pair_address, "pair_created_at": pair_created_at,
        "pair_created_block": pair_block, "last_scanned_block": max(cursor - 1, pair_block),
        "transfer_logs": len(transfer_logs), "receipts_checked": receipts_checked,
        "observations": len(observations),
        "eligible": sum(1 for item in observations if item["filter_status"] == "eligible"),
        "excluded": sum(1 for item in observations if item["filter_status"] == "excluded"),
        "stop_reason": stop_reason,
    }


def load_evm_early_buyer_evidence(limit=100):
    """Return ranked, zero-weight EVM early-buyer evidence and token coverage."""
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT wallet,
                    COUNT(DISTINCT token_address) AS early_tokens,
                    COUNT(DISTINCT token_address) FILTER (WHERE entry_rank <= 25) AS top_25_tokens,
                    ROUND(AVG(entry_rank)::numeric, 1) AS average_entry_rank,
                    MIN(entry_rank) AS best_entry_rank,
                    STRING_AGG(DISTINCT token_symbol, ', ' ORDER BY token_symbol) AS token_symbols,
                    MIN(occurred_at) AS first_observed_at,
                    MAX(occurred_at) AS last_observed_at
                FROM evm_early_buyer_observations
                WHERE filter_status = 'eligible'
                GROUP BY wallet
                ORDER BY early_tokens DESC, top_25_tokens DESC,
                    average_entry_rank ASC, first_observed_at ASC
                LIMIT %s
            """, (limit,))
            wallet_rows = cur.fetchall()
            cur.execute("""
                SELECT state.token_address, state.token_symbol, state.pair_address,
                    state.pair_created_at, state.scan_status,
                    state.provider_status, state.observations_retained,
                    COUNT(observation.wallet) FILTER (
                        WHERE observation.filter_status = 'eligible'
                    ) AS eligible_wallets,
                    COUNT(observation.wallet) FILTER (
                        WHERE observation.filter_status = 'excluded'
                    ) AS excluded_wallets,
                    state.updated_at, state.last_error
                FROM evm_early_buyer_token_state state
                LEFT JOIN evm_early_buyer_observations observation
                    ON observation.chain = state.chain
                    AND LOWER(observation.token_address) = LOWER(state.token_address)
                GROUP BY state.token_address, state.token_symbol, state.pair_address,
                    state.pair_created_at, state.scan_status, state.provider_status,
                    state.observations_retained, state.updated_at, state.last_error
                ORDER BY state.updated_at DESC
            """)
            token_rows = cur.fetchall()
            cur.execute("""
                SELECT COUNT(DISTINCT wallet),
                    COUNT(DISTINCT wallet) FILTER (WHERE filter_status = 'eligible'),
                    COUNT(DISTINCT wallet) FILTER (WHERE filter_status = 'excluded'),
                    COUNT(DISTINCT token_address),
                    COUNT(DISTINCT wallet) FILTER (
                        WHERE filter_status = 'eligible' AND wallet IN (
                            SELECT wallet FROM evm_early_buyer_observations
                            WHERE filter_status = 'eligible'
                            GROUP BY wallet HAVING COUNT(DISTINCT token_address) >= 2
                        )
                    )
                FROM evm_early_buyer_observations
            """)
            ...
            cur.execute("""
                SELECT id, started_at, completed_at, tokens_selected,
                    tokens_completed, wallets_observed, status, stop_reason, details
                FROM evm_early_buyer_runs ORDER BY id DESC LIMIT 1
            """)
            run = cur.fetchone()
    wallets = [{
        "wallet": row[0], "early_tokens": row[1], "top_25_tokens": row[2],
        "average_entry_rank": float(row[3]) if row[3] is not None else None,
        "best_entry_rank": row[4], "token_symbols": row[5],
        "first_observed_at": row[6], "last_observed_at": row[7],
        "filter_result": "CROSS-TOKEN REVIEW" if row[1] >= 2 else "COLLECTING",
        "screening_status": "AWAITING DEEP FILTER",
        "consensus_weight": 0,
    } for row in wallet_rows]
    tokens = [{
        "token_address": row[0], "token_symbol": row[1],
        "pair_address": row[2], "pair_created_at": row[3],
        "scan_status": row[4], "provider_status": row[5],
        "observations_retained": row[6], "eligible_wallets": row[7],
        "excluded_wallets": row[8], "updated_at": row[9], "last_error": row[10],
    } for row in token_rows]
    latest_run = None if not run else {
        "id": run[0], "started_at": run[1], "completed_at": run[2],
        "tokens_selected": run[3], "tokens_completed": run[4],
        "wallets_observed": run[5], "status": run[6],
        "stop_reason": run[7], "details": json.loads(run[8] or "{}"),
    }
    return {
        "counts": {"observed": counts[0] or 0, "eligible": counts[1] or 0,
                   "excluded": counts[2] or 0, "tokens_scanned": counts[3] or 0,
                   "cross_token_wallets": counts[4] or 0},
        "wallets": wallets, "tokens": tokens, "latest_run": latest_run,
        "policy": {"consensus_weight": 0, "automatic_admission": False,
                   "same_block_snipers_excluded": True,
                   "pair_event_verification_required": True,
                   "minimum_cross_token_evidence": 2,
                   "deep_filter_required": True},
    }
...
EVM_SIGNAL_SELECT = """
    ...
    JOIN token_watchlist w ON w.chain = s.chain
        AND LOWER(w.token_address) = LOWER(s.token_address)
        AND w.active = TRUE
...
def run_evm_early_buyer_discovery(limit=None):
    ...
    with db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                FROM token_watchlist watchlist
                LEFT JOIN evm_early_buyer_token_state state
                    ON state.chain = watchlist.chain
                    AND LOWER(state.token_address) = LOWER(watchlist.token_address)
                WHERE watchlist.active = TRUE
                    AND watchlist.chain = 'robinhood'
                    AND watchlist.source <> 'benchmark'
                    AND watchlist.canonical_pair_address IS NOT NULL
                ORDER BY CASE WHEN state.token_address IS NULL THEN 0 ELSE 1 END,
                    state.updated_at ASC NULLS FIRST, watchlist.token_symbol
                LIMIT %s
            """, (limit,))
            tokens = cur.fetchall()
            cur.execute("""
                INSERT INTO evm_early_buyer_runs (tokens_selected, details)
                VALUES (%s, %s) RETURNING id
            """, (len(tokens), json.dumps({
                "wallet_limit_per_token": EVM_EARLY_BUYER_LIMIT,
                "receipt_limit_per_token": EVM_EARLY_BUYER_MAX_RECEIPTS,
                "log_block_limit": EVM_EARLY_BUYER_LOG_BLOCKS,
                "consensus_weight": 0,
    ...
    deadline = time.monotonic() + EVM_EARLY_BUYER_MAX_SECONDS
    results, errors = [], []
    stop_reason = None
    for token in tokens:
        if time.monotonic() >= deadline:
            ...
            break
        try:
            result = scan_robinhood_early_buyers(token, deadline)
            results.append(result)
            if result.get("stop_reason") == "deadline_guard":
                stop_reason = "deadline_guard"
                break
        except (requests.RequestException, RuntimeError, ValueError, TypeError) as exc:
            error = str(exc)[:160]
            errors.append({"token_symbol": token[2], "error": error})
            with db() as conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        INSERT INTO evm_early_buyer_token_state (
                            chain, token_address, token_symbol, pair_address,
                            scan_status, provider_status, last_error, updated_at
                        ) VALUES (%s, %s, %s, %s, 'blocked', 'degraded', %s, NOW())
                        ON CONFLICT (chain, token_address) DO UPDATE SET
                            token_symbol = EXCLUDED.token_symbol,
                            pair_address = EXCLUDED.pair_address,
                            scan_status = 'blocked', provider_status = 'degraded',
                            last_error = EXCLUDED.last_error, updated_at = NOW()
                    """, (token[0], token[1], token[2], token[3], error))
                conn.commit()
            if "429" in error or "deadline" in error or "timeout" in error.lower():
                stop_reason = "provider_guard"
                break

    wallets_observed = sum(item.get("observations", 0) for item in results)
    status = (
        "completed" if len(results) == len(tokens) and not errors
        else "bounded_stop" if stop_reason
        else "partial"
    )
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE evm_early_buyer_runs SET completed_at = NOW(),
                    tokens_completed = %s, wallets_observed = %s,
                    status = %s, stop_reason = %s,
                    details = details::jsonb || %s::jsonb
                WHERE id = %s
            """, (
...
def evm_early_buyers_endpoint():
    initialise_database()
    try:
        limit = min(max(int(request.args.get("limit", 100)), 1), 250)
    except ValueError:
        limit = 100
    evidence = load_evm_early_buyer_evidence(limit)
    return jsonify({
        "success": True, "version": VERSION, **evidence,
        "paper_mode": True, "actionable": False,
        "warning": "Early-purchase evidence is observational and cannot vote or trigger admission.",
    })


@app.get("/evm-early-buyer-status")
def evm_early_buyer_status_endpoint():
    initialise_database()
    evidence = load_evm_early_buyer_evidence(25)
    return jsonify({
        "success": True, "version": VERSION,
        "provider": {"name": "Robinhood public JSON-RPC",
                     "configured": bool(ROBINHOOD_RPC_URL)},
        **evidence, "paper_mode": True, "actionable": False,
    })
...
def evm_wallet_holdings_endpoint():
    """Read-only multi-chain holdings and detected add/remove events."""
    initialise_database()
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT wallet, chain, token_address, token_symbol, token_name,
                    raw_balance, decimals, ui_balance, usd_value,
                    discovery_source, is_held, first_checked_at,
                    last_checked_at, last_changed_at
                FROM evm_wallet_holdings
                WHERE wallet IN (%s, %s)
                ORDER BY is_held DESC, chain, token_symbol
            """, (EVM_PORTFOLIO_WALLET, SOLANA_PORTFOLIO_WALLET))
            ...
            cur.execute("""
                ...
                WHERE wallet IN (%s, %s) ORDER BY id DESC LIMIT 50
            """, (EVM_PORTFOLIO_WALLET, SOLANA_PORTFOLIO_WALLET))
    ...
    holdings = [{
        "wallet": row[0], "chain": row[1], "token_address": row[2],
        "token_symbol": row[3], "token_name": row[4],
        "raw_balance": row[5], "decimals": row[6],
        "ui_balance": row[7], "usd_value": row[8],
        "discovery_source": row[9], "is_held": row[10],
        "first_checked_at": row[11], "last_checked_at": row[12],
        "last_changed_at": row[13],
    ...
    return jsonify({
        ...
        "scope": "all discovered nonzero holdings across Robinhood, Base, BSC, and Solana",
    })
...
def build_dashboard_payload():
    ...
    with db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT wallet, chain, token_address, token_symbol, token_name,
                    raw_balance, decimals, ui_balance, usd_value,
                    last_checked_at, discovery_source
                FROM evm_wallet_holdings
                WHERE is_held = TRUE AND (
                    wallet = %s OR (wallet = %s AND chain = 'solana')
                )
                ORDER BY chain, token_symbol
            """, (EVM_PORTFOLIO_WALLET, SOLANA_PORTFOLIO_WALLET))
            holding_rows = cur.fetchall()
    ...
    holding_items = [{
        ...
        "raw_balance": row[5], "decimals": row[6],
        "ui_balance": row[7], "usd_value": row[8],
        "last_checked_at": row[9], "discovery_source": row[10],
    } for row in holding_rows]
    held_by_token = {
        (item["chain"], str(item["token_address"]).lower()): {
            "raw_balance": item["raw_balance"],
            "ui_balance": item["ui_balance"],
            "usd_value": item["usd_value"],
            "last_checked_at": item["last_checked_at"],
        }
        for item in holding_items if item["chain"] in SUPPORTED_EVM_CHAINS
    }
    evm_signals = [
    ...
    for item in evm_signals:
        ...
        item["wallet_ui_balance"] = holding["ui_balance"]
        item["wallet_usd_value"] = holding["usd_value"]
        if (
            item["wallet_usd_value"] is None
            and safe_float(item["wallet_ui_balance"]) is not None
            and safe_float(item.get("price_usd")) is not None
        ):
            item["wallet_usd_value"] = (
                safe_float(item["wallet_ui_balance"])
                * safe_float(item.get("price_usd"))
            )
        item["wallet_balance_checked_at"] = holding["last_checked_at"]

    represented = {
        (item["chain"], str(item["token_address"]).lower()) for item in evm_signals
    }
    empty_trends = {
        window: {
            "available": False, "price_change_pct": None,
            "liquidity_change_pct": None, "holder_change": None,
            "baseline_at": None, "eth_change_pct": None,
            "relative_to_eth_pct": None,
        } for window in ("1h", "6h", "24h")
    }
    for holding in holding_items:
        key = (holding["chain"], str(holding["token_address"]).lower())
        if key in represented:
            continue
        chain = holding["chain"]
        address = holding["token_address"]
        chain_label = (
            "Solana" if chain == "solana"
            else EVM_CHAIN_CONFIG.get(chain, {}).get("label", chain.title())
        )
        ui_balance = safe_float(holding["ui_balance"])
        usd_value = safe_float(holding["usd_value"])
        price_usd = (
            usd_value / ui_balance
            if usd_value is not None and ui_balance not in {None, 0} else None
        )
        is_native = address == "native"
        dex_url = None
        if chain == "solana":
        ...
        evm_signals.append({
            ...
            "token_symbol": holding["token_symbol"] or "UNKNOWN",
            "token_name": holding["token_name"],
            "status": "HELD", "structure_state": "WALLET HOLDING",
            "structure_confidence": 100, "price_usd": price_usd,
            "liquidity_usd": None, "liquidity_tier": "—",
            "holder_count": None, "holder_data_age_seconds": None,
            "volume_h1_usd": None, "data_quality": "wallet balance",
            "trusted_snapshot_age_seconds": None, "is_benchmark": False,
            "dexscreener_url": dex_url, "trends": empty_trends,
            "wallet_raw_balance": holding["raw_balance"],
            "wallet_ui_balance": ui_balance,
            "wallet_usd_value": usd_value,
            "wallet_balance_checked_at": holding["last_checked_at"],
            "discovery_source": holding["discovery_source"],
        })
        represented.add(key)
    ...
    portfolio_holdings = {
        ...
        "refresh_hours": EVM_HOLDINGS_REFRESH_HOURS,
        "counts_by_chain": {
            chain: sum(1 for item in holding_items if item["chain"] == chain)
            for chain in sorted({item["chain"] for item in holding_items})
        },
    ...
    return {
        ...
        "summary": {
            ...
            "portfolio_tokens": len(evm_signals),
            ...
            "evm_early_buyer_wallets": evm_early_buyers["counts"]["eligible"],
            ...
            "recommended_for_probation": candidate_pipeline["counts"][
                "recommended_for_probation"
            ],
        },
        "paper_mode": True, "actionable": False,
    }
...
<body><main class="wrap">
  <header class="top"><div><div class="eyebrow">V4.26 · Complete multi-chain portfolio</div><h1>Wallet Monitor Dashboard</h1><div class="sub">Current Robinhood, Base, BNB Chain and Solana holdings · refreshed twice daily</div></div><div class="live"><span class="dot"></span><span id="refreshState">Loading live data…</span></div></header>
  <section class="cards">
    <div class="card"><span class="label">Wallet-held tokens</span><b id="evmCount">—</b><span class="muted">Robinhood + Base + BNB Chain + Solana</span></div>
  ...
  <section class="section"><div class="section-head"><div><h2>Multi-chain wallet watchlist</h2><div class="muted" id="evmHoldingStatus">Discovering every nonzero holding in both wallets; balances checked twice daily</div></div><div class="links"><a href="/evm-wallet-holdings">Wallet holdings</a><a href="/evm-signals">All JSON signals</a><a href="/evm-transition-history">Transitions</a><a href="/evm-snapshots?limit=100">Snapshots</a><a href="/evm-anomalies">Anomalies</a><a href="/evm-provider-events">Provider events</a></div></div><div class="table-wrap"><table><thead><tr><th>Token / chain</th><th>Wallet balance</th><th>Value</th><th>State</th><th>Structure</th><th>Price</th><th>Liquidity</th><th>Holders</th><th>1h price</th><th>6h price</th><th>24h price</th><th>24h vs ETH</th><th>24h holders</th><th>Volume 1h</th><th>Quality</th></tr></thead><tbody id="evmBody"><tr><td colspan="15" class="empty">Loading…</td></tr></tbody></table></div></section>
...
const balance=v=>v==null?'—':new Intl.NumberFormat('en-US',{notation:Math.abs(v)>=1e6?'compact':'standard',maximumFractionDigits:6}).format(v);
...
@media(max-width:1100px){.cards{grid-template-columns:repeat(2,1fr)}.metrics{grid-template-columns:repeat(3,1fr)}.diag-grid{grid-template-columns:repeat(3,1fr)}}@media(max-width:650px){.wrap{padding:20px 12px 50px}.top{display:block}.live{margin-top:10px}.metrics{grid-template-columns:repeat(2,1fr)}.cards{grid-template-columns:1fr}.diag-grid{grid-template-columns:repeat(2,1fr)}#qualityAltsTable thead{display:none}#qualityAltsTable,#qualityAltsTable tbody,#qualityAltsTable tr,#qualityAltsTable td{display:block;width:100%}#qualityAltsTable tr{padding:16px 18px;border-bottom:1px solid var(--line)}#qualityAltsTable td{border:0;padding:3px 0}#qualityAltsTable td:nth-child(1){font-size:20px;margin-bottom:5px}#qualityAltsTable td:nth-child(2),#qualityAltsTable td:nth-child(3){display:inline-block;width:auto;margin-right:10px}#qualityAltsTable td:nth-child(4){font-size:18px;margin-top:8px}#qualityAltsTable td:nth-child(5),#qualityAltsTable td:nth-child(6),#qualityAltsTable td:nth-child(7){display:inline-block;width:auto;margin-right:12px}#qualityAltsTable td:nth-child(5)::before{content:"24h "}#qualityAltsTable td:nth-child(6)::before{content:"7d "}#qualityAltsTable td:nth-child(7)::before{content:"30d "}#qualityAltsTable td:nth-child(8),#qualityAltsTable td:nth-child(9),#qualityAltsTable td:nth-child(10){display:none}#qualityAltsTable td:nth-child(11){margin-top:7px}}
